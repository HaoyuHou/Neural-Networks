...
class linear_layer:
    ...
    def forward(self, X):
        ...
        ################################################################################
        w=X.dot(self.params['W'])
        
        return self.params['b']+w
        ...
        if hasattr(module, 'params'):
            for key, _ in module.params.items():
                ...
                if _alpha > 0.0:
                    ...
                    val=module_name+'_'+key
                    momentum[val]=_alpha*momentum[val]-_learning_rate*g
                    module.params[key]=module.params[key]+momentum[val]
...
def main(main_params, optimization_type="minibatch_sgd"):
    ...
    for t in range(num_epoch):
        ...
        for i in range(int(np.floor(N_train / minibatch_size))):
            ...
            ######################################################################################
            a1=model['L1'].forward(x)
            a2=model['L2'].forward(model['drop1'].forward(model['nonlinear1'].forward(a1),is_train=False))
